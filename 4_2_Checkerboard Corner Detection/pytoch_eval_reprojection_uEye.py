...
if len(sys.argv)>1 and int(sys.argv[1])==0:
    ...
    for idx_dow in np.range(len(dwn_rate)):
        ...
        for idx_test in range(len(files_list)):
            ...
            if idx_dow==0:
                ...
                dwn_board, gray = OpenCV_corners(files_list[idx_test].replace('/uEye/','/uEye_dwn_{0}/'.format(idx_dow)))
...
if len(sys.argv)>1 and int(sys.argv[1])==1:
    ...
    for idx_dow in np.range(len(dwn_rate)):
        ...
        for idx_test in range(len(files_list)):
            ...
            if idx_dow==0:
                ...
                dwn_board = np.loadtxt(
                    files_list[idx_test].replace('/benchmarkData/uEye/', '/results/uEye_{0}/'.format(idx_dow)).replace(
                        '.png', '_rochade_refined_5.txt'))
                dwn_board -= 1  # matlab
                dwn_board /= dwn_rate[idx_dow]
        ...
        _, gray = OpenCV_corners(files_list[0].replace('/uEye/','/uEye_dwn_{0}/'.format(idx_dow)))
        if not alternative:
            ret, mtx, dist, rvecs, tvecs = cv2.calibrateCamera(objpoints, imgpoints, gray.shape[::-1],None,None)
...
if len(sys.argv)>1 and int(sys.argv[1])==4:
    ...
    for idx_dow in np.range(len(dwn_rate)):
        ...
        for idx_test in range(len(files_list)):
            ...
            if idx_test%50==0 and len(objpoints)>=20:
