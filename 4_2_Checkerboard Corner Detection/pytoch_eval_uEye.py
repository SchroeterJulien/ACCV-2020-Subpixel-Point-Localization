...
for idx_dow in range(len(dwn_rate)):
    ...
    for idx_test in range(len(files_list)):
        ...
        # Original
        oCam_board = \
            np.load(files_list[idx_test].replace('/benchmarkData/uEye/','/results/uEye_original/').replace('.png','_OCamCalib.npy'))

        original_board = getHeatmapCorners(files_list[idx_test])

        dwn_board = getHeatmapCorners(files_list[idx_test].replace('/uEye/','/uEye_dwn_{0}/'.format(idx_dow)))
...
_subpixel = True # whether to use subpixel refinement or argmax
...
for idx_dow in range(len(dwn_rate)):
    ...
    for idx_test in range(len(files_list)):
        ...
        dwn_board = OpenCV_corners(files_list[idx_test].replace('/uEye/','/uEye_dwn_{0}/'.format(idx_dow)))
        dwn_board /= dwn_rate[idx_dow]
...
for idx_dow in range(len(dwn_rate)):
    ...
    for idx_test in range(len(files_list)):
        ...
        original_board = \
            np.loadtxt(files_list[idx_test].replace('/benchmarkData/uEye/','/results/uEye_original/').replace('.png','_rochade_refined_5.txt'))
        original_board-=1 #matlab

        dwn_board = np.loadtxt(
            files_list[idx_test].replace('/benchmarkData/uEye/', '/results/uEye_{0}/'.format(idx_dow)).replace('.png','_rochade_refined_5.txt'))
        dwn_board-=1 # matlab
...
for idx_dow in range(3):
    ...
    for idx_test in range(len(files_list)):
        ...
        dwn_board = np.load(
            files_list[idx_test].replace('/benchmarkData/uEye/', '/results/uEye_{0}/'.format(idx_dow)).replace('.png','_OCamCalib.npy'))
